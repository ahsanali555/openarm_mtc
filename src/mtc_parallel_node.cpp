...
void MTCTaskNode::setupPlanningScene()
{
  ...
  stick.primitives[0].dimensions = { 0.2, 0.02 };   // [height, radius]
    ...
    mtc::Stage* current_state_ptr, mtc::Stage** attach_stage_out)
{
  ...
  // grasp pose generation + IK
  {
    ...
    Eigen::Matrix3d rot;
    rot.col(0) = -Eigen::Vector3d::UnitX();  // flip X
    rot.col(1) =  Eigen::Vector3d::UnitY();  // keep Y
    rot.col(2) = -Eigen::Vector3d::UnitZ();  // flip Z → faces down
    grasp_frame_transform.linear() = rot;
...
mtc::Task MTCTaskNode::createTask()
{
  ...
  // debugging left.
  const Eigen::Vector3d RIGHT_GRASP_AXIS = Eigen::Vector3d::UnitZ();
  const double RIGHT_GRASP_ANGLE = M_PI / 2;
  const double RIGHT_GRASP_Z_OFFSET = 0.2;
  ...
  const Eigen::Vector3d LEFT_GRASP_AXIS = Eigen::Vector3d::UnitZ();   // <-- try this first
  ...
  task.add(buildPlaceContainer("right", right_arm, right_gripper, right_ee, right_hand,
                                0.3, 0.3, cartesian_planner, interpolation_planner,
                                right_attach_stage));
