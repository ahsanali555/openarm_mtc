...
def moveit_nodes_spawner(context: LaunchContext, arm_type, use_fake_hardware):
    ...
    # THE ONE THING THIS FILE ADDS on top of demo.launch.py: move_group needs
    # this capability loaded to expose the ExecuteTaskSolution action --
    # that's what task_.execute(...) in mtc_node.cpp actually calls. Without
    # it, MTC solutions can plan fine but have nowhere to send execution to.
    # Kept in this MTC-specific bringup file rather than added to the shared
    # demo.launch.py, since it's only relevant when MTC is involved.
    ...
    # Prefer our own saved MTC-specific RViz config (once you've saved one --
    # see config/README_RVIZ.txt) so the Motion Planning Tasks display and
    # its /solution topic don't need re-adding on every relaunch. Falls back
    # to the shared moveit.rviz until that file exists.
    own_pkg_path = get_package_share_directory("openarm_mtc_pick_place")
    own_rviz_cfg = os.path.join(own_pkg_path, "config", "mtc.rviz")
    shared_rviz_cfg = os.path.join(moveit_pkg_path, "config", config_dir, "moveit.rviz")
    rviz_cfg = own_rviz_cfg if os.path.exists(own_rviz_cfg) else shared_rviz_cfg
    ...
    return [
        Node(
            ...
            output="screen",
            parameters=[moveit_params, move_group_capabilities],
        ),
        Node(
            package="rviz2",
            executable="rviz2",
